...
import { Currency, CurrencyAmount, SwapParameters, Token, Trade, TradeOptions, TradeType } from 'eotc-bscswap-sdk'
...
export default function v1SwapArguments(trade: Trade, options: Omit<TradeOptions, 'feeOnTransfer'>): SwapParameters {
  ...
  const inputETH = trade.inputAmount.currency === Currency.ETHER
  const outputETH = trade.outputAmount.currency === Currency.ETHER
  if (inputETH && outputETH) throw new Error('Currency.ETHER to Currency.ETHER')
